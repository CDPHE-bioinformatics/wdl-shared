...
import "../version_capture/structs.wdl"

task scrub_reads_hostile {
  meta {
    description: "Perform read scrubbing with hostile"
    tool: "https://github.com/bede/hostile"
  }

  parameter_meta {
    fastq1: "Either single FASTQ or paired R1 FASTQ"
    fastq2: "Optional- if exists, paired R2 FASTQ"
    genome_index: {
      options: {
        illumina: "human-t2t-hla.fa.gz"
        oxford_nanopore: ["human-t2t-hla.1.bt2", "human-t2t-hla.2.bt2", "human-t2t-hla.3.bt2", "human-t2t-hla.4.bt2", "human-t2t-hla.rev.1.bt2", "human-t2t-hla.rev.2.bt2"]
      }
      help: "Path(s) to genome index file(s)"
    }
    seq_method: {
      options: ["ILLUMINA", "OXFORD_NANOPORE"]
      help: "Uses bowtie2 and minimap2, respectively"
    }
  }

...
struct SubdirsToFiles {
    ...
    meta {
        description: "Array of relative subdirectories and files to transfer to them"
    }
    parameter_meta {
        subdirs_to_files {
            example: "[('fastq_scrubbed', [hostile.fastq1_scrubbed, hostile.fastq2_scrubbed]), ('seqyclean', [seqyclean.seqyclean_summary])]"
        }
    }
}
...
task transfer {
    meta {
        description: "Transfer files to Google Cloud Storage"
    }

    parameter_meta {
        cpu: "Default 1"
        out_dir: "Parent-level GCS directory to send files to. Includes gs:// prefix"
        overwrite: "Boolean- if you would like to overwrite existing files"
        subdirs_to_files: "Custom struct- `SubdirsToFiles subdirs_to_files = object { subdirs_to_files: [<array>]}`"
    }
